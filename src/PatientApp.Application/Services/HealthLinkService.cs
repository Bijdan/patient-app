...
public class HealthLinkService : IHealthLinkService
{
    ...
    public async Task<SmartHealthLinkDto> ProcessBundleAsync(string bundleJson, string baseUrl)
    {
        ...
        // 5. Build and persist the submission entity (Id auto-generated by MongoDB)
        ...
        // 6. Write encrypted files to disk using the generated Id
        var bundlePath = $"{submission.Id}/bundle.enc";
        var pdfPath = $"{submission.Id}/document.enc";
        ...
        // 7. Update file paths on the submission
        submission.BundleFilePath = bundlePath;
        submission.PdfFilePath = pdfPath;
        await _repository.UpdateAsync(submission);

        // 8. Build the SHL response
        var retrievalUrl = $"{baseUrl.TrimEnd('/')}/api/v1/healthlinks/{submission.Id}";
